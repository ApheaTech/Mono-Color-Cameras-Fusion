...
class RetinexNet(nn.Module):
    ...
    def forward(self, input_low, input_high):
        # Forward DecompNet
        input_color = Variable(torch.FloatTensor(torch.from_numpy(input_low))).cuda()
        input_mono = Variable(torch.FloatTensor(torch.from_numpy(input_high))).cuda()
        R_color, I_color = self.DecomNet(input_color)
        R_mono, I_mono = self.DecomNet(input_mono)

        # Forward RelightNet
        # TODO 对两对图像均进行relight
        I_delta_color = self.RelightNet(I_color, R_color)
        I_delta_mono = self.RelightNet(I_mono, R_mono)

        # self.illumination_map = I_delta.detach().cpu()

        # Other variables
        I_color_3 = torch.cat((I_color, I_color, I_color), dim=1)
        I_mono_3 = torch.cat((I_mono, I_mono, I_mono), dim=1)
        I_delta_color_3 = torch.cat((I_delta_color, I_delta_color, I_delta_color), dim=1)
        I_delta_mono_3 = torch.cat((I_delta_mono, I_delta_mono, I_delta_mono), dim=1)

        # TODO Compute losses
        recon_loss_color = F.l1_loss(R_color * I_color_3, input_color)
        recon_loss_mono = F.l1_loss(R_mono * I_mono_3, input_mono)
        ...
        equal_R_loss = SSIM(R_color, R_mono)

        # self.relight_loss = F.l1_loss(R_low * I_delta_3, input_high)
        relight_color_loss = F.l1_loss(R_color * I_delta_color, input_color_label)
        relight_mono_loss = F.l1_loss(R_mono * I_delta_mono_3, input_mono_label)


        Ismooth_loss_color = self.smooth(I_color, R_color)
        Ismooth_loss_mono = self.smooth(I_mono, R_mono)
        Ismooth_loss_delta_color = self.smooth(I_delta_color, R_color)
        Ismooth_loss_delta_mono = self.smooth(I_delta_mono, R_mono)

        self.loss_Decom = recon_loss_color + \
                          recon_loss_mono + \
                          0.1 * Ismooth_loss_color + \
                          0.1 * Ismooth_loss_mono + \
                          0.01 * equal_R_loss
        self.loss_Relight = relight_color_loss + \
                            relight_mono_loss + \
                            3 * Ismooth_loss_delta_color + \
                            3 * Ismooth_loss_delta_mono
    ...
    def predict(self,
        ...
        for idx in range(len(test_low_data_names)):
            ...
            illumination_map = self.illumination_map.numpy().squeeze()
            b = np.full_like(illumination_map, illumination_map.min())
            # illumination_map = (illumination_map - np.full_like(illumination_map, illumination_map.min)) / (np.full_like(illumination_map, illumination_map.max) - np.full_like(illumination_map, illumination_map.min)) * 255
            illumination = ((illumination_map - illumination_map.min()) / (
                        illumination_map.max() - illumination_map.min()) * 255).astype('uint16')
            illumination = Image.fromarray(illumination).convert('RGB')
            illumination.save('illumination_map.png')
