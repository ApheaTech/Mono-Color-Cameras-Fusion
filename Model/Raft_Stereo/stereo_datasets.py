...
class SceneFlowDatasets(StereoDataset):
    def __init__(self, aug_params=None, root='/home/xuhang/dataset/SceneFlow/', dstype='frames_cleanpass', things_test=False):
    ...
    def _add_monkaa(self):
        ...
        left_images = sorted(glob(osp.join(root, self.dstype, '*/left/*.webp')))
...
class KITTI(StereoDataset):
    def __init__(self, aug_params=None, root='datasets/KITTI', image_set='training'):
        ...
        disp_list = sorted(
            glob(os.path.join(root, 'training', 'disp_occ_0/*_10.png'))) if image_set == 'training' else [osp.join(root,
                                                                                                                   'training/disp_occ_0/000085_10.png')] * len(
            image1_list)
...
class Middlebury(StereoDataset):
    def __init__(self, aug_params=None, root='/home/xuhang/dataset/Middlebury', split='Q'):
        ...
        if split == "2014":  # datasets/Middlebury/2014/Pipes-perfect/im0.png
            ...
            image1_list = sorted(
                [os.path.join(root, "MiddEval3", f'training{split}', f'{name}/im0.png') for name in lines])
            image2_list = sorted(
                [os.path.join(root, "MiddEval3", f'training{split}', f'{name}/im1.png') for name in lines])
            disp_list = sorted(
